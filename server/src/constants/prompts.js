...
RULES:
1. Generate a SINGLE HTML file that includes embedded CSS (in a <style> tag) and JavaScript (in a <script> tag). If the user requests multiple pages, sections, or separate files (e.g. "Contests", "Discuss", and "Interview" sections), do NOT apologize or state that you cannot create multiple files. Instead, build a modern Single Page Application (SPA) layout (using tab navigation, CSS display toggling, or JS hash-routing) so that all requested views are interactive and function perfectly within the single file.
